 ...
 * ── WHY ONLY THREE ─────────────────────────────────────────────────────────
 *
 * The original list had six, three of which pointed at Retreat and Sponsor
 * pages that do not exist on main and are not authorised. A destination key
 * naming a page nobody has built is a printed QR code waiting to 404, so the
 * list holds only what a scan can actually reach today. Adding a key later is
 * one line; un-printing a postcard is not.
 *
 * All three were also on the retired /life-is-a-project slug, which has
 * redirected permanently since 22 August. A partner QR would have taken a 302
 * to a 308 to the real page — functional, and the wrong thing to commit to
 * paper.
 */
