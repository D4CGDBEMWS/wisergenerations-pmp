...
>>>>>>> 3c0c3a8ba5a6108351a83bbfd0cc408e0cff98e2
} as const
...
<<<<<<< HEAD
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (result.error) {
      setErrorMessage(
        result.error.message ?? 'Unable to process payment. Please review your details and try again.',
      )
  ...
  return (
    ...
    >
      <div>
        <h2 className="text-2xl font-semibold text-slate-900">Complete your payment</h2>
...
export default function CheckoutPage() {
  ...
  async function handleContinueToPayment(event: FormEvent<HTMLFormElement>) {
    ...
    if (stripeUnavailable) {
      setFormError(
        'Stripe is not configured. Add NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY before accepting live payments.',
      )
      return
    }
        ...
        <section className="space-y-6">
          <div className="space-y-3">
            ...
            <p className="max-w-2xl text-base leading-7 text-slate-600">
              Choose the program that matches your next milestone. Pricing is validated
              server-side and payment details stay inside Stripe&apos;s PCI-compliant flow.
            </p>
          </div>
        ...
        <aside className="space-y-6">
          ...
          {clientSecret && stripePromise ? (
            ...
            <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-6 text-sm leading-6 text-slate-600 shadow-sm">
              Your secure Stripe Payment Element will appear here after you submit your
              enrollment details.
            </div>
          )}

          <div className="rounded-2xl bg-slate-900 p-6 text-sm leading-6 text-slate-200 shadow-sm">
            <h2 className="text-lg font-semibold text-white">Need help before enrolling?</h2>
            <p className="mt-2">
              If you have questions about which program is right for you, contact the Wiser
              Generations team before completing checkout.
            </p>
